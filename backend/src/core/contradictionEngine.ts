...
export type ContradictionResult = {
  ...
  reason?: string;
  path?: string; // for deep checks (optional)
};
 ...
 */
export function findFirstContradiction(
  items: any[]
): ContradictionResult & { indexA?: number; indexB?: number } {
 ...
 */
export function findContradictions(statements: string[]): ContradictionResult[] {

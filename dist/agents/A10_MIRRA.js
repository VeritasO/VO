...
function performContradictionScan(caseContext) {
...
function assessReflectionRequirements(caseContext, contradictionScan) {
    let depth = "standard";
    let urgency = "normal";
    let status = "ready";
    // Determine reflection depth needed
    if (contradictionScan.contradictions.some((c) => c.severity === "high")) {
        depth = "deep";
        urgency = "high";
    }
...
function identifySystemicContradictions(caseContext) {
    const patterns = [];
    // Look for recurring contradiction types
    if (caseContext.contradictions) {
        const contradictionTypes = caseContext.contradictions.map((c) => c.summary);
        const typeFrequency = contradictionTypes.reduce((freq, type) => {
...
function analyzeDoctrintalGaps(caseContext, doctrine) {
    const gaps = [];
...
function performDoctrinalReflection(contradiction, doctrine, reflectionNeeds) {
...
function checkAgentCoordination(involvedAgents) {
    // Simplified coordination check
    return involvedAgents.length > 5 ? ["too_many_agents"] : [];
}
function analyzeAgentInvolvementPattern(involvedAgents) {
    return {
...
//# sourceMappingURL=A10_MIRRA.js.map
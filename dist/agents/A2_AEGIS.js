...
exports.AEGIS = {
    ...
    async act({ caseContext, doctrine, memory }) {
        ...
        if (biasAssessment.biasDetected) {
            ...
            const adjustedSeverity = severity === "critical" ? "high" : severity;
            const biasContradiction = (0, contradictionUtils_1.createContradiction)(`Bias detected: ${biasAssessment.biasType} - ${biasAssessment.description}`, adjustedSeverity, [this.id], [1, 2] // Books I & II violations
            );
            return {
        ...
        if (fairnessAudit.concernsDetected) {
            ...
            fairnessAudit.concerns.forEach((concern) => {
        ...
        if (factualConsistency.inconsistenciesFound) {
            ...
            factualConsistency.inconsistencies.forEach((inconsistency) => {
...
function performBiasAssessment(caseContext) {
    let biasScore = 0;
...
function conductFairnessAudit(caseContext) {
...
function assessProportionality(caseContext) {
...
function verifyFactualConsistency(caseContext) {
...
function detectSystemicBias(caseContext) {
    let riskLevel = "low";
    const patterns = [];
...
function determineBiasSeverity(biasAssessment) {
    if (biasAssessment.biasScore > 0.7)
        return "critical";
    if (biasAssessment.biasScore > 0.4)
        return "high";
    if (biasAssessment.biasScore > 0.2)
        return "medium";
    return "low";
}
// Helper functions (simplified implementations)
function analyzeDemographics(participants) {
    // Simplified demographic analysis
    return { imbalanceDetected: Math.random() < 0.1 };
}
function analyzeOutcomeBias(historicalOutcomes) {
    // Simplified outcome bias analysis
    return { biasDetected: Math.random() < 0.05 };
}
function getHarmWeight(harmLevel) {
    const weights = { low: 1, medium: 2, high: 3, critical: 4 };
    return weights[harmLevel] || 2;
}
function getResponseWeight(responseLevel) {
    const weights = { minimal: 1, moderate: 2, significant: 3, severe: 4 };
    return weights[responseLevel] || 2;
}
function findContradictoryStatements(statements) {
    // Simplified contradiction detection
    return statements.filter(s => s.contradictory) || [];
}
function checkTimelineConsistency(timeline) {
    // Simplified timeline consistency check
    return [];
}
function verifyEvidenceConsistency(evidence) {
    // Simplified evidence consistency check
    return [];
}
function analyzeHistoricalBiasPatterns(caseHistory) {
    return { patternDetected: false };
}
function assessInstitutionalBias(institutionalFactors) {
    return { detected: false };
}
function detectAlgorithmicBias(algorithmicProcessing) {
    return { detected: false };
}
//# sourceMappingURL=A2_AEGIS.js.map
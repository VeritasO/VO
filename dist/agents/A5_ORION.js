...
function analyzeRightsFramework(caseContext, doctrine) {
...
function validateOntologicalConsistency(caseContext) {
    ...
    entities.forEach((entity, index) => {
        if (entity.type && definitions[entity.type]) {
            ...
            if (!entity.properties || !definition.required_properties) {
                ...
                const missingProps = definition.required_properties.filter((prop) => !entity.properties.hasOwnProperty(prop));
...
function assessSovereigntyRisks(caseContext) {
    const risks = [];
    const decisions = caseContext.decisions || [];
...
function identifyAppliedFrameworks(caseContext) {
    const frameworks = [];
    if (caseContext.human_rights_considered)
        frameworks.push("human_rights");
    if (caseContext.indigenous_frameworks)
        frameworks.push("indigenous_rights");
    if (caseContext.intersectional_analysis)
        frameworks.push("intersectional");
    if (caseContext.environmental_frameworks)
        frameworks.push("environmental");
    if (caseContext.community_rights)
        frameworks.push("community_rights");
    return frameworks;
}
function identifyRightsAtStake(caseContext) {
    const rights = [];
    if (caseContext.affects_privacy)
        rights.push("privacy");
    if (caseContext.affects_autonomy)
        rights.push("autonomy");
    if (caseContext.affects_expression)
        rights.push("expression");
    if (caseContext.affects_cultural_practice)
        rights.push("cultural");
    if (caseContext.affects_land_access)
        rights.push("land");
    if (caseContext.affects_community_bonds)
        rights.push("community");
    return rights;
}
function generateSovereigntySafeguards(risks) {
    const safeguards = [];
...
//# sourceMappingURL=A5_ORION.js.map
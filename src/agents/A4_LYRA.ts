...
function analyzeNarratives(caseContext: any) {
...
function gatherTestimonies(caseContext: any) {
...
function identifyNarrativeGaps(narrativeAnalysis: any, testimonies: any[]) {
  const gaps = [];
...
function validateEmotionalSovereignty(testimonies: any[]) {
...
function calculateAuthenticity(narratives: any[]) {
  if (narratives.length === 0) return 100;
  
  // Simple authenticity calculation based on narrative elements
  const authenticityScores = narratives.map(narrative => {
    let score = 50; // base score
    
    if (narrative.personal_details) score += 20;
    if (narrative.emotional_content) score += 15;
    if (narrative.cultural_markers) score += 10;
    if (narrative.specific_details) score += 5;
...
function assessNarrativeCoherence(narratives: any[]) {
  if (narratives.length === 0) return 1;
  
  // Simple coherence assessment
  const timelineConsistent = narratives.every(n => n.timestamp || n.sequence);
  const themesConsistent = narratives.length < 2 || 
    narratives.every(n => n.themes && n.themes.some((theme: string) => 
      narratives[0].themes?.includes(theme)));
  
  let coherenceScore = 0.5; // base
  if (timelineConsistent) coherenceScore += 0.25;
  if (themesConsistent) coherenceScore += 0.25;
  
  return coherenceScore;
}

function assessEmotionalDepth(narratives: any[]) {
  if (narratives.length === 0) return 0.5;
